...
import { apiGetAll, apiPrint, apiUpdate } from "../../services/admin/sallebill.services";
...
const SalleBill: React.FC = () => {
    ...
    const handlePrint = async () => {
        if (selectedRowKeys.length > 0) {
            let results = await apiPrint({ data: selectedRowKeys });

            const fileURL = window.URL.createObjectURL(results);
